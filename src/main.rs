fn main() {
    // println!("{:?}", [4, 14].map(|size| std::fs::read("input.txt").unwrap().windows(size).take_while(|w| std::collections::HashSet::<&u8>::from_iter(*w).len() != size).count() + size))
    // let a = [0,1,,2,];
...
// fn day6(input: &str, size: usize) -> usize {
//     // let a = input.as_bytes().windows(size).take_while(|w| std::collections::HashSet::<&u8>::from_iter(*w));
//     todo!()
    
//     // input.chars().collect::<Vec<_>>().windows(size).take_while(|w| {HashSet::<&char>::from_iter(*w).len() != size}).count() + size
// }
